import{i as k}from"./assets/support-dc4a270f.js";import"./assets/vendor-310c93a9.js";function m(){return{divEl:document.querySelector(".filled-basket"),addBtnEL:document.querySelector(".shop_card-btn")}}function B(){return{bookShopIconPath:new URL("data:image/png;base64,",self.location).href,amazonIconPath:new URL("data:image/png;base64,",self.location).href,emptyListImage:new URL("/project-js-group-1-g2g/assets/books-desk@1x-c7720492.png",self.location).href,svgTrashIcon:new URL("/project-js-group-1-g2g/assets/icons-55a1aa95.svg",self.location)}}const A="books",o=JSON.parse(localStorage.getItem(A))||[],L=document.querySelector(".blank-basket"),{divEl:a,previousButton:M,paginationPagesStart:l}=m(),{bookShopIconPath:f,amazonIconPath:b}=B(),r=3;let I=1,c=(I-1)*r,F=c+r,C=o.slice(c,F);function p(e){return e.map(({_id:t,title:s,author:i,description:d,list_name:h,book_image:E,amazon_product_url:n})=>`
                <article class="shop_card">
                    <div class="card-img">
                        <img class="shop__card-img" src="${E}" alt="${s}" />
                    </div>

                    <div class="card-title">
                        <h3 class="shop__book-title">${s}</h3>
                        <p class="shop__book-category">${h}</p>
                    </div>

                    <div class="card-description">
                        <p class="shop__book-description">${d}</p>
                    </div>

                    <div class="card-author">
                        <p class="shop__book-author">${i}</p>
                    </div>

                    <div class="card-shoplist">
                        <ul class="shop__book-shoplist">
                            <li class="store">
                                <a href="${n}" target="_blank" rel="noopener noreferrer nofollow" aria-label="Amazon link">
                                    <img class="modal-shop-img shopping-shopimg amazon" src="${b}" alt="Amazon link" alt="Amazon live page"/>
                                </a>
                            </li>
                            <li class="store">
                                <a href="${n}" target="_blank" rel="noopener noreferrer nofollow" aria-label="BookShop link">
                                    <img class="modal-shop-img shopping-shopimg book-shop" src="${f}" alt="BookShop link" />
                                </a>
                            </li>
                        </ul>
                    </div>
                    <button class="delete-btn" type="button" data-book-id="${t}" aria-label="Remove book from shopping list">
                        <svg class="trash-icon" data-book-id="${t}" width="18" height="18">
                            <use href="${k}#trash-icon"></use>
                        </svg>
                    </button>
                </article>
                `).join("")}function g(){if(!o.length){L.classList.remove("display-none");return}a.insertAdjacentHTML("beforeend",p(C))}g();a.addEventListener("click",e=>{if(e.target.closest(".delete-btn")){const t=e.target.getAttribute("data-book-id"),s=o.findIndex(i=>i._id===t);if(o.splice(s,1),localStorage.setItem(A,JSON.stringify(o)),o.length)sliceArrayBooks().length?(a.innerHTML=p(sliceArrayBooks()),console.log(sliceArrayBooks()),destroyChildElement(l)):(M.click(),destroyChildElement(l));else return a.innerHTML="",g();checkingArrayBooks()}});
//# sourceMappingURL=commonHelpers2.js.map
