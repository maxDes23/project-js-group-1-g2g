import"./assets/support-f0aae234.js";import"./assets/vendor-ddb081ef.js";function k(){return{divEl:document.querySelector(".blank-basket"),addBtnEL:document.querySelector(".shop_card-btn")}}function m(){return{bookShopIconPath:new URL("data:image/png;base64,",self.location).href,amazonIconPath:new URL("data:image/png;base64,",self.location).href,emptyListImage:new URL("/project-js-group-1-g2g/assets/books-desk@1x-c7720492.png",self.location).href,svgTrashIcon:new URL("/project-js-group-1-g2g/assets/icons-430406ff.svg",self.location)}}const A="books",o=JSON.parse(localStorage.getItem(A))||[],{divEl:e,previousButton:B,paginationPagesStart:n}=k(),{bookShopIconPath:L,amazonIconPath:M,svgTrashIcon:f}=m(),l=3;let b=1,r=(b-1)*l,I=r+l,F=o.slice(r,I);function c(t){return t.map(({_id:s,title:a,author:i,description:p,list_name:g,book_image:d,amazon_product_url:h,buy_links:[E]})=>`
                <article class="shop_card">
                    <div class="card-img">
                        <img class="shop__card-img" src="${d}" alt="${a}" />
                    </div>

                    <div class="card-title">
                        <h3 class="shop__book-title">${a}</h3>
                        <p class="shop__book-category">${g}</p>
                    </div>

                    <div class="card-description">
                        <p class="shop__book-description">${p}</p>
                    </div>

                    <div class="card-author">
                        <p class="shop__book-author">${i}</p>
                    </div>

                    <div class="card-shoplist">
                        <ul class="shop__book-shoplist">
                            <li class="store">
                                <a href="${h}" target="_blank" rel="noopener noreferrer nofollow" aria-label="Amazon link">
                                    <img class="modal-shop-img shopping-shopimg amazon" src="${M}" alt="Amazon link" alt="Amazon live page"/>
                                </a>
                            </li>
                            <li class="store">
                                <a href="${E.url}" target="_blank" rel="noopener noreferrer nofollow" aria-label="BookShop link">
                                    <img class="modal-shop-img shopping-shopimg book-shop" src="${L}" alt="BookShop link" />
                                </a>
                            </li>
                        </ul>
                    </div>
                    <button class="shop_card-btn" type="button" data-book-id="${s}" aria-label="Remove book from shopping list">
                        <svg class="trash-icon" data-book-id="${s}" width="17" height="17">
                            <use href="${f}#trash-icon"></use>
                        </svg>
                    </button>
                </article>
                `).join("")}function C(){if(!o.length){e.innerHTML="";return}e.insertAdjacentHTML("beforeend",c(F))}C();e.addEventListener("click",t=>{if(t.target.closest(".shop_card-btn")){const s=t.target.getAttribute("data-book-id"),a=o.findIndex(i=>i._id===s);if(o.splice(a,1),localStorage.setItem(A,JSON.stringify(o)),o.length)sliceArrayBooks().length?(e.innerHTML=c(sliceArrayBooks()),console.log(sliceArrayBooks()),destroyChildElement(n)):(B.click(),destroyChildElement(n));else{e.innerHTML="";return}checkingArrayBooks()}});
//# sourceMappingURL=commonHelpers2.js.map
