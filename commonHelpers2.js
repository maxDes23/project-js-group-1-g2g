import{i as d}from"./assets/support-264ca447.js";import"./assets/vendor-ab85f73e.js";function E(){return{divEl:document.querySelector(".filled-basket"),addBtnEL:document.querySelector(".shop_card-btn")}}function h(){return{bookShopIconPath:new URL("data:image/png;base64,",self.location).href,amazonIconPath:new URL("data:image/png;base64,",self.location).href,emptyListImage:new URL("/project-js-group-1-g2g/assets/books-desk@1x-c7720492.png",self.location).href,svgTrashIcon:new URL("/project-js-group-1-g2g/assets/icons-55a1aa95.svg",self.location)}}const k="books";let t=JSON.parse(localStorage.getItem(k))||[];const m=document.querySelector(".blank-basket"),{divEl:a,previousButton:u,paginationPagesStart:R}=E(),{bookShopIconPath:B,amazonIconPath:L}=h(),A=6;let M=1,l=(M-1)*A,b=l+A;t.slice(l,b);function f(o){a.innerHTML=o.map(({id:e,title:s,author:r,description:c,list_name:p,book_image:g,amazon_product_url:i})=>`
                <article class="shop_card">
                    <div class="card-img">
                        <img class="shop__card-img" src="${g}" alt="${s}" />
                    </div>

                    <div class="card-title">
                        <h3 class="shop__book-title">${s}</h3>
                        <p class="shop__book-category">${p}</p>
                    </div>

                    <div class="card-description">
                        <p class="shop__book-description">${c}</p>
                    </div>

                    <div class="card-author">
                        <p class="shop__book-author">${r}</p>
                    </div>

                    <div class="card-shoplist">
                        <ul class="shop__book-shoplist">
                            <li class="store">
                                <a href="${i}" target="_blank" rel="noopener noreferrer nofollow" aria-label="Amazon link">
                                    <img class="modal-shop-img shopping-shopimg amazon" src="${L}" alt="Amazon link" alt="Amazon live page"/>
                                </a>
                            </li>
                            <li class="store">
                                <a href="${i}" target="_blank" rel="noopener noreferrer nofollow" aria-label="BookShop link">
                                    <img class="modal-shop-img shopping-shopimg book-shop" src="${B}" alt="BookShop link" />
                                </a>
                            </li>
                        </ul>
                    </div>
                    <button class="delete-btn" type="button" data-book-id="${e}" aria-label="Remove book from shopping list">
                        <svg class="trash-icon" data-book-id="${e}" width="18" height="18">
                            <use class="t-ico" data-book-id="${e}" href="${d}#trash-icon"></use>
                        </svg>
                    </button>
                </article>
                `).join("")}function n(){return t.length?f(t):(a.innerHTML="",m.classList.remove("display-none"))}n();a.addEventListener("click",F);function F(o){(o.target.closest(".delete-btn")||o.target.closest(".trash-icon")||o.target.closest(".t-ico"))&&(t=t.filter(e=>e.id!=o.target.getAttribute("data-book-id")),localStorage.setItem("books",JSON.stringify(t))),n()}
//# sourceMappingURL=commonHelpers2.js.map
