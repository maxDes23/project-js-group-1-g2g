import{i as E}from"./assets/support-d5be5692.js";import"./assets/vendor-ab85f73e.js";function h(){return{divEl:document.querySelector(".filled-basket"),addBtnEL:document.querySelector(".shop_card-btn")}}function k(){return{bookShopIconPath:new URL("data:image/png;base64,",self.location).href,amazonIconPath:new URL("data:image/png;base64,",self.location).href,emptyListImage:new URL("/project-js-group-1-g2g/assets/books-desk@1x-c7720492.png",self.location).href,svgTrashIcon:new URL("/project-js-group-1-g2g/assets/icons-55a1aa95.svg",self.location)}}const l="books";let o=JSON.parse(localStorage.getItem(l))||[];const m=document.querySelector(".blank-basket"),{divEl:s,previousButton:R,paginationPagesStart:w}=h(),{bookShopIconPath:B,amazonIconPath:L}=k(),A=6;let f=1,r=(f-1)*A,b=r+A;o.slice(r,b);function M(t){s.innerHTML=t.map(({id:e,title:i,author:c,description:p,list_name:g,book_image:d,amazon_product_url:n})=>`
                <article class="shop_card">
                    <div class="card-img">
                        <img class="shop__card-img" src="${d}" alt="${i}" />
                    </div>

                    <div class="card-title">
                        <h3 class="shop__book-title">${i}</h3>
                        <p class="shop__book-category">${g}</p>
                    </div>

                    <div class="card-description">
                        <p class="shop__book-description">${p}</p>
                    </div>

                    <div class="card-author">
                        <p class="shop__book-author">${c}</p>
                    </div>

                    <div class="card-shoplist">
                        <ul class="shop__book-shoplist">
                            <li class="store">
                                <a href="${n}" target="_blank" rel="noopener noreferrer nofollow" aria-label="Amazon link">
                                    <img class="modal-shop-img shopping-shopimg amazon" src="${L}" alt="Amazon link" alt="Amazon live page"/>
                                </a>
                            </li>
                            <li class="store">
                                <a href="${n}" target="_blank" rel="noopener noreferrer nofollow" aria-label="BookShop link">
                                    <img class="modal-shop-img shopping-shopimg book-shop" src="${B}" alt="BookShop link" />
                                </a>
                            </li>
                        </ul>
                    </div>
                    <button class="delete-btn" type="button" data-book-id="${e}" aria-label="Remove book from shopping list">
                        <svg class="trash-icon" data-book-id="${e}" width="18" height="18">
                            <use href="${E}#trash-icon"></use>
                        </svg>
                    </button>
                </article>
                `).join("")}function a(){return o.length?M(o):(s.innerHTML="",m.classList.remove("display-none"))}a();s.addEventListener("click",u);function u(t){t.target.closest(".delete-btn")&&(console.log(o),o=o.filter(e=>e.id!=t.target.getAttribute("data-book-id")),console.log(o),localStorage.setItem("books",JSON.stringify(o)),a()),a()}function F(t){return o.some(e=>e.id===t)}document.addEventListener("add-to-cart",t=>{const e=t.detail.book;F(e.id)||(o.push(e),localStorage.setItem(l,JSON.stringify(o)))});
//# sourceMappingURL=commonHelpers2.js.map
