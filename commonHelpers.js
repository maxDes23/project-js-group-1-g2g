import"./assets/shopping-list-829390ef.js";import{a as i,i as b,g as k,c as f,s as v}from"./assets/vendor-8a3563eb.js";i.defaults.baseURL="https://books-backend.p.goit.global/books/";async function h(){return await i.get("category-list")}async function w(){return await i.get("top-books")}async function g(t){return await i.get(`category?category=${t}`)}async function L(t){return await i.get(`${t}`)}document.addEventListener("DOMContentLoaded",async function(){const t=await h(),{data:e}=t;S(e)});function S(t){const e=document.querySelector(".categories"),n=t.map(o=>{const{list_name:s}=o;return`
        <li><a class='categories_item' href='#' id='${s}'>${s}</a></li>
        `}).join("");e.insertAdjacentHTML("beforeend",n),[...document.getElementsByClassName("categories_item")].forEach(o=>o.addEventListener("click",()=>{document.querySelector(".active_category").classList.remove("active_category"),o.classList.add("active_category")}))}async function C(t){try{const{book_image:e,title:n,author:a,list_name:o,amazon_product_url:s}=t,c=document.querySelector(".modal");console.log("modal Constan",c),c.innerHTML="";const d=['<button class="modal-close-button">Close</button>',`<img src="${e}" class="modal-image">`,`<h2 class="modal__title">${n}</h2>`,`<p class="modal-title-name">Author: ${a}</p>`,`<p class="modal-title-text">List Name: ${o}</p>`,`<a href="${s}" class="modal-link">Amazon Link</a>`,'<button class="modal-button-add">ADD</button>'];c.innerHTML=d.join(""),document.querySelector(".modal-close-button").addEventListener("click",()=>{c.classList.remove("active")}),c.classList.add("active")}catch(e){console.log(e)}}document.addEventListener("DOMContentLoaded",async function(){const t=await w(),{data:e}=t;E(e),I(),document.querySelector(".books-container").addEventListener("click",a);async function a(o){const s=o.target.closest(".book");if(!s)return;const c=s.id,d=await L(c);C(d.data)}});function u(t){return document.querySelector(".books-container"),t.map(n=>{const{_id:a,book_image:o,title:s,author:c}=n;return`
      <div class="book" id="${a}">
